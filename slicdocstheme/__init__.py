...
_giturl = 'https://git.slic.org/cgit/{}/tree/doc/source'
...
def builder_inited(app):
    ...
    app.info('Using slicdocstheme Sphinx theme from %s' % theme_dir)
...
def get_pdf_theme_path():
    ...
    args = ['theme', 'slicdocs_pdf', 'pdftheme']
    return os.path.join(get_pkg_path(), *args)


def get_slic_logo_path():
    """Return the directory containing slic logo for local builds."""
    args = ['theme', 'slicdocs_pdf', 'slic-logo-full.png']
    return os.path.join(get_pkg_path(), *args)
...
def setup(app):
    app.info('connecting events for slicdocstheme')
    ...
    app.add_html_theme(
        'slicdocs',
        os.path.abspath(os.path.dirname(__file__)) + '/theme/slicdocs',
    )
    return {
